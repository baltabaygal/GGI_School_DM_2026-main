...
class CosmiXs:
    ...
    def _mass_finalstate(self):
        if self.final_state == 'AntiP':
            return 0.93827208
        elif (self.final_state == 'Gamma') | (self.final_state == 'Nuel') | (self.final_state == 'Numu') | (self.final_state == 'Nuta'):
            return 0.
        elif self.final_state == 'Positrons':
            return 0.000510998950
...
#            print("DEBUG columns:", df.columns.tolist())
    ...
    def _linear_interpolation(self):
        ...
        mfs = self._mass_finalstate()
        if len(data) == 2:
            return 10**np.array(data[0])*self.mass+mfs, gaussian_filter1d(data[1]/(10**np.array(data[0])*self.mass)/np.log(10), sigma=2)
        elif len(data) == 6:
            dNdLog10x = []
            for i in range(len(data[1])):
                dNdLog10x.append(self._linear_function(data[3], data[0], data[2][i], data[5][i]))
            return 10**np.array(data[1])*self.mass+mfs, gaussian_filter1d(dNdLog10x/(10**np.array(data[1])*self.mass)/np.log(10), sigma=2)

    # A dataframe is being made that will be the final output.
    def _generate_dataframe_output(self, f):
        return pd.DataFrame(data = {'E [GeV]': f[0], 'dNdE [1/GeV]': f[1]})
...
def annihilation_spectrum(mass, channel, final_state, path=None):
    ...
    """
    spectrum = CosmiXs(mass, channel, final_state, path)
    return spectrum.make_spectrum().to_numpy()